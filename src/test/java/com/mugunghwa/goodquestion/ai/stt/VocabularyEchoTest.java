...
class VocabularyEchoTest {
    ...
    private VocabularyEchoDetector detector(String hint) {
        return new VocabularyEchoDetector(hint);
    }

    @Test
    void 힌트_전체를_그대로_반복하면_에코다() {
        assertThat(detector(HINT).isEcho(
    ...
    void 힌트_일부만_나열해도_에코다() {
        assertThat(detector(HINT).isEcho("며느리, 시아버지, 방귀")).isTrue();
        assertThat(detector(HINT).isEcho("배나무 장대")).isTrue();
    }
    ...
    void 힌트_단어가_들어간_정상_발화는_에코가_아니다() {
        assertThat(detector(HINT).isEcho("며느리가 방귀를 뀌어서 배가 떨어졌어요")).isFalse();
        assertThat(detector(HINT).isEcho("시아버지한테 솔직하게 말하는 게 좋을 것 같아요")).isFalse();
    }

    /** 아이가 한 단어로 답하는 것은 정상이다. 나열 판정은 힌트 단어 2개부터만 적용한다. */
    @Test
    void 힌트_단어_하나만_말한_것은_에코가_아니다() {
        assertThat(detector(HINT).isEcho("방귀!")).isFalse();
        assertThat(detector(HINT).isEcho("며느리")).isFalse();
    }

    @Test
    void 힌트_단어_두어_개에_실제_내용이_붙으면_에코가_아니다() {
        assertThat(detector(HINT).isEcho("방귀 뀌는 며느리가 계속 참았어요")).isFalse();
    }
    ...
    void 힌트가_없거나_결과가_null이면_판정하지_않는다() {
        assertThat(detector("").isEcho("며느리, 시아버지, 방귀")).isFalse();
        assertThat(detector(HINT).isEcho(null)).isFalse();
    }
}
