...
-- ------------------------------------------------------------
-- 23. 흐름 확인용 이야기 "작은 씨앗" - 삭제됐다
--     대화 턴 파이프라인(POST /utterances)이 501이던 시절, STORY 장면만으로 이루어진
--     이야기를 하나 둬서 후속 활동과 별가루 지급까지 실제로 이어지는지 확인하던 용도였다.
--     파이프라인이 붙어 방귀 뀌는 며느리로 같은 흐름을 다 볼 수 있으므로 역할이 끝났다.
--     PUBLISHED라 실제 사용자 목록에도 노출되고 있어서 함께 내렸다.
--     이미 적재된 DB의 행은 V19__remove_demo_seed_story.sql이 지운다.
-- ------------------------------------------------------------
