...
--
-- Flyway가 V1 다음에 자동 적용한다. 한 번 적용된 뒤에는 수정하지 않는다 —
-- 체크섬이 어긋나면 앱이 뜨지 않는다. 시드를 바꿔야 하면 V3를 새로 추가한다.
-- 트랜잭션 제어(begin/commit)는 Flyway가 감싸므로 파일에 두지 않는다.
--
-- ⚠ TODO(배포 전): 이 파일은 운영 DB에도 그대로 적용된다.
--   데모 계정(demo@goodquestion.kr)과 가짜 진행 기록까지 운영에 들어가므로,
--   배포 시점에 데모분을 지우는 V3를 추가하거나 콘텐츠/데모를 분리한다.
--   지금은 배포 전이라 그대로 둔다.
--
-- 데모 로그인
