...
public class CharacterPromptBuilder {
    ...
    public String systemPrompt(CharacterLlmClient.CharacterLlmInput input) {
        ...
        if ("CLOSING".equals(input.mode())) {
            // 최대 턴 종료 턴의 짧은 반응. 마무리 대사는 고정 문장으로 따로 이어지므로
            // 여기서는 반응만 하고 정리하지 않는다 (콘텐츠 문서 "짧게 반응 -> 마지막 대사").
            prompt.append("""

                    [대사 규칙 - 장면이 끝나는 턴]
                    - 아이의 마지막 말에 1~2문장으로 따뜻하게 반응만 한다.
                    - 평가나 지적을 하지 않는다. 아이가 말을 잘 못했더라도 아쉬움을 드러내지 않는다.
                    - 새로운 질문을 하지 않는다. 아이가 대답할 차례는 이제 없다.
                    - 작별 인사나 정리를 하지 않는다. 마무리 대사는 따로 정해져 있고 네 반응 바로 뒤에 이어진다.
                    - 아이의 이름을 부르지 않는다. 아이의 말을 평가하는 점수, 정답, 학습 같은 말을 쓰지 않는다.
        ...
        if (input.remainingWorry() != null && !input.remainingWorry().isBlank()) {
            if (input.softCue()) {
                ...
                prompt.append("""
                        ...
                        답안을 캐릭터가 먼저 말하지 않는다.
                        대사는 물음 하나로 끝낸다. 물음은 걱정이 담고 있는 상황을 짚되, 지식을
                        확인하는 학습 질문처럼 들리게 하지 않는다.
                        물음은 시점을 하나만 잡는다. 아이에게 묻거나("너라면 이럴 때 어떻게 할
                        것 같아?"), 캐릭터가 혼잣말로 묻거나("그런데 다음에도 같은 일이 생기면
                        어떡하지?") 둘 중 하나다. 한 물음 안에서 두 시점을 겹치지 않는다.
                        (깨진 예: "너는 내가 그렇게 생각하는 모습 보면 어떨까?" — 아이를 주어로
                         세워 놓고 캐릭터의 자문으로 끝맺어 말이 되지 않는다.)
                        (안 되는 물음: "해결 방법을 말해 봐.")
                        아이가 방금 무언가를 물었으면 그 물음에 먼저 한 문장으로 답한 뒤 걱정을 꺼낸다.
                        아래 걱정에 이미 물음이 들어 있으면 그 물음을 그대로 쓰거나 자연스럽게
                        다듬어 쓴다 — 새 물음을 만들어 뒤에 덧붙이지 않는다.
                        한 대사에 물음표는 하나만 쓴다.
                        걱정: %s
                        """.formatted(input.remainingWorry()));
            }
