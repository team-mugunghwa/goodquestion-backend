        ...
        if (input.remainingWorry() != null && !input.remainingWorry().isBlank()) {
            if (input.softCue()) {
                ...
                prompt.append("""
                        ...
                        물음은 아래 두 형태 중 하나만 쓰고, 한 문장 안에서 섞지 않는다.
                          - 아이에게 묻기(주어가 아이): "너라면 이럴 때 어떻게 할 것 같아?"
                          - 캐릭터의 혼잣말(주어가 캐릭터): "그런데 다음에도 같은 일이 생기면 어떡하지?"
                        아이와 캐릭터를 한 물음의 주어로 겹쳐 세우지 않는다.
                        (깨진 예: "너는 내가 그렇게 생각하는 모습 보면 어떨까?" — 아이를 주어로
                         세워 놓고 캐릭터의 자문으로 끝맺어 말이 되지 않는다.)
                        (안 되는 물음: "해결 방법을 말해 봐.")
                        아래 걱정에 이미 물음이 들어 있으면 그 물음을 살려 끝내고 새 물음을 덧붙이지
                        않는다. 한 대사에 물음표는 하나만 쓴다.
                        걱정: %s
                        """.formatted(input.remainingWorry()));
            }
