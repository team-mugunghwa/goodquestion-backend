...
**왜 접속 기록이 마지막 1건인가** - `last_login_ip`와 `last_login_at`은 이력을 쌓지 않고 덮어쓴다. 관리자 콘솔이 묻는 것은 "이 보호자가 마지막으로 언제 왔는가"라서 행을 쌓을 이유가 없고, 계정을 지우면 함께 사라져 별도 파기 절차도 생기지 않는다. 날짜별 방문 추이는 `daily_visits`가 따로 센다.
