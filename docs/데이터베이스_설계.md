...
| 28 | `first_login_bonus_grants` | 최초 로그인 별가루를 지급한 계정(계정당 1행, V23) |
| **자유 대화(V21)** | | |
| 26 | `free_talks` | 완주 후 인물과의 자유 대화 한 판 |
| 27 | `free_talk_messages` | 자유 대화의 발화(텍스트만) |
| **공통** | | |
| 25 | `idempotent_requests` | 멱등키 기록(V6) - 발화/구매/자유 대화의 재전송 중복 방지 |
...
|---|---|---|---|---|
| `id` | uuid | PK | | V1 |
| `email` | varchar(255) | 부분 UNIQUE | 이메일 계정만 값이 있다 | V1 |
| `password_hash` | varchar(100) | | bcrypt(60자). 소셜 계정은 null | V1 |
| `provider` | varchar(20) | NOT NULL | `LOCAL` / `KAKAO` / `GOOGLE` | V1, V4 |
| `provider_id` | varchar(100) | | 소셜 제공자 측 식별자 | V1 |
| `name` | varchar(50) | NOT NULL | | V1 |
| `failed_login_attempts` | smallint | NOT NULL, 기본 0 | 연속 실패 횟수. 성공하면 0으로 돌아간다 | V2 |
| `locked_until` | timestamptz | | 잠금 해제 시각. null이거나 과거면 로그인할 수 있다 | V2 |
| `last_login_ip` | varchar(45) | | 마지막으로 로그인한 위치. IPv6가 최대 45자 | V2 |
| `last_login_at` | timestamptz | | 마지막으로 접속한 시각 | V20 |
| `status` | varchar(20) | NOT NULL, CHECK | `ACTIVE` / `SUSPENDED`. 관리자 콘솔이 바꾼다 | V12 |
| `suspended_at` | timestamptz | | 정지시킨 시각 | V12 |
| `suspended_reason` | text | | 정지 사유. 없으면 나중에 왜 막았는지 아무도 모른다 | V12 |
| `created_at` | timestamptz | NOT NULL | | V1 |
...
**왜 접속 기록이 마지막 1건인가** - `last_login_ip`와 `last_login_at`은 이력을 쌓지 않고 덮어쓴다. 관리자 콘솔이 묻는 것은 "이 보호자가 마지막으로 언제 왔는가"라서 행을 쌓을 이유가 없고, 계정을 지우면 함께 사라져 별도 파기 절차도 생기지 않는다. 날짜별 방문 추이는 `daily_visits`가 따로 센다. 무엇을 남기고 무엇을 남기지 않기로 했는지는 [접속_기록_처리.md](접속_기록_처리.md)에 정리했다.

**왜 재발급도 접속으로 세는가** - `last_login_at`은 로그인(이메일/소셜/가입)뿐 아니라 리프레시 토큰 재발급에도 갱신된다. 액세스 토큰이 30분, 리프레시가 14일이라 며칠 만에 앱을 연 사용자는 비밀번호 없이 재발급으로 들어온다. 로그인만 세면 그 사람의 마지막 접속 시각이 2주 전에서 멈춘다. 반대로 `last_login_ip`는 재발급에서 갱신하지 않는다 - "마지막으로 로그인한 위치"라는 뜻은 자격 증명을 실제로 확인한 때만 성립한다.

**왜 `if not exists`인가** - `status`(V12)와 `last_login_at`(V20)은 관리자 콘솔(admin-goodquestion-backend)도 읽는 컬럼이라 양쪽 리포지터리에 같은 마이그레이션이 들어 있다. 두 앱이 같은 DB에 각자 Flyway를 돌리므로 먼저 뜬 쪽이 컬럼을 만들고 나중 쪽은 건너뛴다.
...
| `display_order` | smallint | NOT NULL, 기본 100 | 목록과 홈의 노출 순서. 작을수록 앞 |
| `created_at` | timestamptz | NOT NULL | |

**노출 순서는 `display_order` 오름차순이 먼저고 `created_at` 내림차순이 그다음이다.** 시드는
`created_at`을 적지 않아 DB 기본값 `now()`가 들어가는데, 이 값은 트랜잭션 시작 시각이라 한
마이그레이션에서 함께 들어간 이야기끼리 같아진다. `created_at`만으로는 첫 칸을 고정할 수
없어 순서를 콘텐츠가 정하도록 컬럼을 따로 뒀다.
...
| `conflict` | text | | 갈등 요약 |
| `image_url` | text | | |
| `video_url` | varchar(255) | | 장면 영상(V17). null이면 image_url만 쓴다. 영상이 있어도 image_url은 폴백용으로 유지 |
...
**STT 신뢰도 판정은 저장 시 애플리케이션이 한다.** 기준값 0.5(2026-08 확정, `external.stt.low-confidence-threshold`로 조정). 신뢰도는 exp(토큰 logprob 평균) 0~1이고, 원값도 함께 저장해 기준값이 바뀌면 소급 판정할 수 있다.
...
UNIQUE로 중복 제출을 막는 설계였다. **현재 이 테이블은 휴면 상태다(2026-08-15)** -
미션 제출이 발화 경로(utterances + missionId)로 확정되면서 별도 결과 제출 API와
409 `MISSION_ALREADY_SUBMITTED`가 제거됐고, 쓰고 읽는 코드가 없다. 질문별
구조화 저장이 필요해지면 발화 요청에 선택 필드를 얹어 여기 저장한다.

---

### 4.5 `free_talks` / `free_talk_messages` (V21)

이야기를 완주한 뒤 그 이야기의 인물과 이어서 하는 대화다. **학습이 아니라 관계**라서
요소 판정도 유도도 별가루도 없고 리포트에도 들어가지 않는다.
...
| `turn_count` | smallint | NOT NULL DEFAULT 0, CHECK >= 0 | 아이가 말한 횟수. 캐릭터 대사는 세지 않는다 |
| `ended_at` | timestamptz | | 값이 있으면 닫힌 대화 |
| `created_at` | timestamptz | NOT NULL | |

인덱스: `(child_id, story_id)` - 인물 고르기 화면의 "마지막으로 이야기한 때" 조회.
...
|---|---|---|---|
| `id` | uuid | PK | |
| `free_talk_id` | uuid | FK->free_talks, NOT NULL | |
| `role` | varchar(10) | NOT NULL, CHECK | `CHILD` / `CHARACTER` |
| `text` | text | NOT NULL | |
| `emotion` | varchar(20) | | 캐릭터 발화만 |
| `turn_order` | smallint | NOT NULL, CHECK >= 0 | UNIQUE(free_talk_id, turn_order). 0은 캐릭터의 첫 인사 |
| `created_at` | timestamptz | NOT NULL | |

**`story_sessions`/`messages`에 얹지 않은 이유** - 얹었다면 리포트와 보상이 읽는 모든
조회에 "자유 대화는 빼고"라는 조건이 붙었을 것이고, 한 군데만 빠뜨려도 학습 지표가
조용히 오염된다. 표를 가르면 그 조건 자체가 필요 없다.

**`turn_order`를 둔 이유** - `created_at`만으로는 순서가 정해지지 않는다. PostgreSQL의
`now()`는 트랜잭션 시작 시각이라 같은 트랜잭션에서 저장된 두 줄이 같은 값을 갖는다.
이 이력은 LLM 입력이라 순서가 뒤집히면 문맥이 통째로 어긋난다. 유니크 제약은 덤으로
동시 제출을 막는다(`messages`의 `turn_order`와 같은 방식).

**완주 판정을 복사해 두지 않는다.** 진입 조건("그 이야기를 완주한 아이")은 조회 시점에
`story_sessions`의 `COMPLETED` 세션으로 판정한다. 같은 사실을 두 곳에 두면 갈린다.

**아이 음성 원본은 저장하지 않는다.** STT 신뢰도도 남기지 않는다 - 그 값은 리포트가
대표 발화를 고를 때 쓰는데 자유 대화는 리포트에 들어가지 않는다.
...
| `example_sentence` | text | | 1) 이야기 예문 - 아이가 단어를 만난 문장 |
| `example_daily` | varchar(300) | | 2) 일상 예문 (V14) |
| `example_advanced` | varchar(300) | | 3) 심화 예문 - 일상보다 한 단계 어려움 (V14) |
| `entry_type` | varchar(20) | NOT NULL, CHECK | `UNKNOWN` / `FAVORITE` |
| `source_scene_id` | uuid | FK->story_scenes | |
| `created_at` | timestamptz | NOT NULL | |

`word`는 **표제어**(조사를 뗀 원형)로 저장한다. 저장 경로가 형태소 분석
(`WordLemmatizer`, Nori)으로 정규화한 뒤 UNIQUE(child_id, word) 중복 검사를
하므로 "기왓장이"와 "기왓장을"은 같은 단어다. (V13 이전에 조사째 저장된
행이 소수 있을 수 있다)

### 5.3-1 `story_vocabulary` (V13)

이야기 단위 단어장의 원본 사전이다. 고정 대사의 낱말은 시드로 관리되는
폐집합이라 뜻을 미리 검수해 둘 수 있고, 단어 담기는 뜻 생성 LLM을 부르기
전에 여기를 먼저 조회한다. 시드(R__4)로만 채워지는 읽기 전용 콘텐츠다.
설계 배경은 프론트 `docs/단어_저장_비용_속도_설계_조사.md`.

| 컬럼 | 타입 | 제약 | 설명 |
|---|---|---|---|
| `id` | uuid | PK | |
| `story_id` | uuid | FK->stories, NOT NULL | |
| `word` | varchar(50) | NOT NULL | 표제어. UNIQUE(story_id, word) |
| `meaning` | varchar(200) | NOT NULL | 검수된 아이 눈높이 뜻 |
| `example_sentence` | varchar(300) | | 1) 이야기 예문 |
| `example_daily` | varchar(300) | | 2) 일상 예문 (V14) |
| `example_advanced` | varchar(300) | | 3) 심화 예문 (V14) |
| `created_at` | timestamptz | NOT NULL | |
...
|---|---|
| 이야기 완주 | +3 |
| 최초 로그인 | 계정당 1회, 그 계정의 아이 전원에게 각 +100 |
...
### 6.2.1 `first_login_bonus_grants`

| 컬럼 | 타입 | 제약 | 설명 |
|---|---|---|---|
| `parent_id` | uuid | PK, FK->parents ON DELETE CASCADE | 계정 자체가 키 |
| `granted_at` | timestamptz | NOT NULL DEFAULT now() | |

최초 로그인 별가루(`FIRST_LOGIN`, +100)를 **계정당 1회**만 주기 위한 선점 기록이다.

지갑은 아이당인데 로그인은 보호자 단위라, "계정당 1회"를 `stardust_transactions`만으로는
표현할 수 없다 - 나중에 아이가 늘면 그 아이에게는 이력이 없어 다시 대상이 된다.
계정 단위 기록을 따로 둬야 하는 이유다. 선점은 `on conflict do nothing`으로 하고
삽입 여부(1/0)로 승패를 가린다(`idempotent_requests`와 같은 방식) - 같은 계정으로 두
로그인이 동시에 들어와도 한 번만 나간다.

**아이가 없는 계정은 행을 남기지 않는다.** 가입 직후 첫 로그인은 아이를 만들기 전이라,
거기서 기록만 남기면 지급 없이 기회가 소진된다. 아이가 생긴 뒤 첫 로그인이 실제 지급
시점이고, 행이 있다는 것은 별가루가 실제로 나갔다는 뜻이다.

지급은 로그인 트랜잭션이 **커밋된 뒤 새 트랜잭션**에서 한다(`FirstLoginBonusListener`).
보너스를 넣다 실패했다고 로그인이 막히면 안 되기 때문이고, 실패하면 선점도 함께 롤백돼
다음 로그인에 다시 시도된다.
...
**MVP 16종** - 지형 소품 6(1원, ALWAYS) / 식물 4(2원, ALWAYS) / 구조물 3(울타리/표지판 2원 ALWAYS, 집 3원 누적 3) / 동물 3(3원: 강아지는 방귀 이야기 완주, 토끼/거북이는 누적 4/5). 가격과 해금은 이야기 1편의 평생 획득(최대 6)에 맞춰 인하했다(2026-08).
...
| **발화 턴 처리** | 트랜잭션 3분할: 준비(읽기) / 아이 발화 + 분석 + 판단 반영 / 캐릭터 대사 + 이동. LLM 호출은 트랜잭션 밖(트러블슈팅 문서 5-3절). 세션 낙관적 락(`version`) + `messages(session_id, turn_order)` 유니크 |
| **재전송 멱등** | 발화/구매는 `Idempotency-Key` 헤더를 `idempotent_requests`(endpoint + scope + key 유니크)에 선점 기록. 완료 응답 재생, 처리 중 409, 24시간 보관 |
...
| 15 | `story_scenes.character_persona` 제거 (V5) | `characters.personality` + `scene_stance`와 내용이 완전히 겹쳤다 |
...
| **토끼/거북이 해금** | 후속 이야기가 없어 누적 4/5로 임시 설정(2026-08 인하) | 이야기 추가 시 `STORY_COMPLETE`로 전환 |
