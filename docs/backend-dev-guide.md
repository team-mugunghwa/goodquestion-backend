...
엔드포인트 56개 중 38개가 동작하고, 4개는 일부 경로만 동작하며, 14개가 501 스텁이다
...
- **동작**: 인증(가입/로그인/카카오), 아이와 동의 전체, 콘텐츠 조회, 홈, 세션 8건 전체,
  리포트 조회, 단어 목록과 즐겨찾기, 보상 11건 전체(상점·별가루·행성 배치)
...
- **501 스텁**: 대화 턴 4건, 후속 활동 4건, 리포트 생성, 단어 삭제, 토큰 재발급과
  로그아웃, 음성 2건

남은 14건 중 **벤더 선정에 걸린 것은 4건**(발화 제출, 리포트 생성, STT, TTS)이다.
후속 활동 4건은 벤더와 무관하고, 이것이 붙어야 완주 시 별가루 지급 경로가 이어진다.
...
| 5 | `learning.postactivity` | 완주 시 별가루 지급 경로가 여기서 이어진다. 보상은 준비돼 있다 |
