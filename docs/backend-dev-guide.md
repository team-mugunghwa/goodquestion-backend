...
**왜 개발 DB를 쓰지 않고 매번 새로 띄우나**

각자 개발 DB에 붙이면 테스트가 통과하는지가 그 사람 DB 상태에 달리게 된다.
손으로 추가한 컬럼, 예전 스키마 위에 얹힌 흔적, 남아 있는 데이터가 전부 결과에 섞인다.
빈 DB에서 마이그레이션만으로 스키마를 다시 만들면 "마이그레이션 파일만 갖고 처음부터
띄웠을 때 실제로 뜨는가"를 매번 검사하게 된다. 새로 합류한 팀원과 배포 환경이 겪는
상황이 정확히 그것이다.
...
### bootRun은 되는데 test가 안 될 때

`bootRun`은 각자 개발 DB에 붙고 `test`는 빈 DB를 새로 만든다. 보는 DB가 다르므로
한쪽만 실패할 수 있다. 원인은 둘 중 하나이고 대응이 정반대라 먼저 구분해야 한다.

**Docker를 못 찾겠다는 에러라면** 환경 문제다. 아래 `-PlocalDb`로 돌리면 된다.

**Flyway나 스키마 검증 에러라면 개발 DB가 어긋난 것이다.** 예를 들어 컬럼을 손으로
추가해 뒀으면 `bootRun`은 통과한다. 그 DB에 이미 컬럼이 있어서 `ddl-auto=validate`가
넘어가기 때문이다. 반면 빈 컨테이너는 `V1__init_schema.sql`부터 새로 만드니 그 컬럼이
없어 실패한다.

이때 틀린 쪽은 테스트가 아니라 개발 DB다. 마이그레이션 파일만 갖고 스키마를 만드는
새 팀원과 배포 환경에서는 실제로 깨진다. 여기서 `-PlocalDb`를 붙여 넘어가면
그 사실을 덮는 것이니 하지 않는다. 로컬 스키마를 비우고 Flyway가 다시 만들게 한 뒤
(3절 "로컬 DB 초기화") 마이그레이션 파일 쪽을 고친다.

### Docker 없이 로컬 PostgreSQL로 돌리기

Docker를 쓰지 않고 직접 설치한 PostgreSQL을 쓰는 경우를 위한 옵션이다.
Docker가 없다는 이유일 때만 쓴다. 테스트가 실패해서 통과시키려고 쓰는 것이 아니다
(위 "bootRun은 되는데 test가 안 될 때" 참고).
...
- 스키마가 마이그레이션 결과와 어긋나 있어도 드러나지 않는다. 빈 DB에서 다시 만드는
  검사가 사라지기 때문이고, 이게 기본값을 컨테이너로 둔 이유다.

기본값은 Testcontainers다. 특별한 이유가 없으면 `-PlocalDb` 없이 쓰는 쪽을 권한다.
CI도 기본 경로로 돈다. 즉 `-PlocalDb`로만 통과하는 코드는 CI에서 걸린다.
...
엔드포인트 56개 중 25개가 동작하고, 3개는 일부 경로만 동작하며, 28개가 501 스텁이다
(2026-08-11 기준). 스텁도 **DTO 계약은 확정**돼 있어 프론트는 미리 붙여 둘 수 있다.
영역별 집계와 개별 상태는 [API-DTO.md 7절](API-DTO.md)에 있다.

대략적인 그림은 이렇다.

- **동작**: 인증(가입/로그인/카카오), 아이와 동의 전체, 콘텐츠 조회, 홈, 세션 8건 전체
  (시작, 조회, 이어하기, 대화 내역, 첫 대사 재생, 현재 장면 조회, 장면 전환, 중단),
  단어 목록과 즐겨찾기, 리포트 조회 2건
...
- **501 스텁**: 대화 턴 파이프라인 전체, 미션, 후속 활동, 리포트 생성, 보상(상점/별가루/행성)
  전체, 토큰 재발급과 로그아웃, 단어 삭제
- **경계에서 막힘**: `/api/stt`, `/api/tts`는 컨트롤러와 서비스까지 구현됐고 벤더 클라이언트만
  비어 있다. 호출하면 501이 오므로 집계에서는 스텁으로 센다

세션 흐름이 다 열려서 **대화 턴을 뺀 재생 경로는 끝까지 이어진다.** 이야기를 고르고 시작해
장면을 넘기고 앱을 껐다 켜도 복구된다. 남은 것은 `POST /utterances` 하나다.

남은 25건 중 **벤더 선정에 걸린 것은 4건**이다. `POST /utterances`, 리포트 생성,
`/api/stt`, `/api/tts`이고 여기에 단어 저장의 LLM 경로가 더해진다. 나머지 21건은 벤더와
무관하다. 특히 보상 11건과 후속 활동 4건은 엔티티와 스키마가 준비돼 있어 지금 바로 착수할 수 있다.
...
| 4 | `learning.postactivity` -> `report` 생성 | 세션 완료 이후 흐름. 리포트 조회 2건은 구현돼 있어 생성만 채우면 된다 |
