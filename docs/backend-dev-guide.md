...
엔드포인트 56개 중 25개가 동작하고 29개가 501 스텁이다. 스텁도 **DTO 계약은 확정**돼 있어
프론트는 미리 붙여 둘 수 있다. 영역별 집계와 개별 상태는
[API-DTO.md 7절](API-DTO.md)에 있다.

대략적인 그림은 이렇다.

- **동작**: 인증(가입/로그인/카카오), 아이와 동의 전체, 콘텐츠 조회, 홈, 세션 시작과 장면 전환, 단어 목록
- **501**: 대화 턴 파이프라인 전체, 미션, 후속 활동, 리포트, 보상(상점/별가루/행성) 전체, 토큰 재발급과 로그아웃
- **경계에서 막힘**: `/api/stt`, `/api/tts`는 서비스까지 구현됐고 벤더 클라이언트만 비어 있다
