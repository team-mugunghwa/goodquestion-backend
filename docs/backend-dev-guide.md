...
> [API와_DTO_명세.md](API와_DTO_명세.md), 테이블과 컬럼은 [데이터베이스_설계.md](데이터베이스_설계.md)가 원본이다.
...
| 알고 싶은 것 | 볼 문서 |
| --- | --- |
| 엔드포인트 경로, 요청/응답 필드, 오류 코드, 구현 상태 | [API와_DTO_명세.md](API와_DTO_명세.md) |
...
3. 문서끼리 다르면 API와_DTO_명세.md와 데이터베이스_설계.md가 이 문서보다 최신이다.
...
세부 엔드포인트와 필드는 [API와_DTO_명세.md](API와_DTO_명세.md)에 있다. 여기서는 어느 클래스부터
읽으면 되는지만 짚는다.
...
1. [API와_DTO_명세.md](API와_DTO_명세.md)에 요청/응답 계약이 있는지 먼저 본다. 있으면 그대로 따른다
...
7. 계약이 바뀌면 [API와_DTO_명세.md](API와_DTO_명세.md)를 함께 고친다

**응답 설계 원칙 6가지**(판정은 서버가, 파생값은 저장하지 않고 계산, 서버 내부 설정은
내리지 않음, 분기는 null로, 정답은 내리지 않음, 발화 원문은 안전 응답에 담지 않음)는
[API와_DTO_명세.md 5절](API와_DTO_명세.md)에 근거와 함께 정리돼 있다.
...
영역별 집계와 개별 상태는 [API와_DTO_명세.md 7절](API와_DTO_명세.md)에 있다.
...
각 문서의 미해결 항목을 합친 것이다. 근거는 [API와_DTO_명세.md 6절](API와_DTO_명세.md),
