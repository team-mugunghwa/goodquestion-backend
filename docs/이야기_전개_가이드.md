...
- 요소를 다 채우면 턴 수와 무관하게 즉시 닫힌다. preferred_turns는 종료 게이트가
  아니라 권장 대화 길이다(충족조건 문서 확정, 2026-08-15 반영). 게이트로 쓰면 1턴에
  요소를 다 채운 아이에게 캐릭터가 물을 것 없는 턴이 하나 더 생긴다
...
| preferred_turns | 문서에 수치 없음. 최소 턴 게이트 개념은 대화작동규칙에 있으나 "정확한 내부 임계값은 운영 정책에 따라 관리"로 위임됨 | **전 장면 2로 확정.** 이후 충족조건 문서(2026-08-15 확정)가 역할을 "최소 턴 게이트"가 아니라 "권장 대화 길이"로 정리해 종료 판정에서는 쓰지 않는다 - 게이트로 쓰면 1턴에 요소를 다 채운 아이에게 유도 불가능한 무목적 턴이 생긴다. 값 자체는 콘텐츠 메타로 유지한다 |
