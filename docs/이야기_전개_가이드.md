...
3. **TTS 계약**: 캐릭터 메시지의 `audioUrl`이 null이면 `POST /api/tts`로 합성한다.
   2026-08-16부터 고정 첫/마지막 대사에는 사전 렌더 음성이 실려 온다 - null 검사
   분기(`audioUrl ?? 합성`)가 그대로 동작하며, LLM 생성 대사와 이름 치환 문장은
   여전히 null이다
...
`imageUrl`로 배경을 그린다. 이 화면에서는 마이크를 비활성화한다.

**영상(2026-08-16부터)**: `currentScene.videoUrl`이 있으면 배경을 이미지 대신 영상으로
그린다. 영상은 이미지를 대체하지 않고 얹는 것이다 - null이거나 재생에 실패하면
(저사양 기기, 데이터 절약 모드 포함) `imageUrl`로 폴백한다. 반복 재생 플래그는 따로
없다 - `sceneType`이 그 정보다. STORY는 낭독이 끝나면 멈추고, DIALOGUE는 아이가
말하는 동안 계속 반복한다.

**음성(2026-08-16부터)**: `currentScene.narrationAudioUrl`이 있으면 그것을 재생하고,
문장 넘김은 `narrationTimings`(문장별 실측 시작/끝 초)를 따른다 - 글자수 비례
타이머는 소리와 자막이 어긋난다. `narrationAudioUrl`이 null이면 기존처럼 문장별로
`POST /api/tts { text }`를 호출한다(characterName 없이 -> 내레이션 보이스).
...
```
POST /api/stt
multipart/form-data, 파트명 "audio" (한도 10MB. 웹은 48kHz 녹음이라 109초부터
한도를 넘으므로 프론트가 60초에서 자동 종료 후 전송한다)
-> 200 { text, rawText, confidence, lowConfidence }
```

- 422 `STT_EMPTY_TEXT`: 무음이거나 인식 실패, 어휘 힌트 에코로 판정된
  결과(트러블슈팅_STT_어휘_힌트_에코.md), 또는 한글이 전혀 없는 결과(무음에서
  영어 상투구를 뱉는 환각). 메시지가 생성되지 않았으니
  "다시 말해 볼까?" 안내 후 재녹음. 이때 재시도 횟수를 세어 둔다(sttRetryCount)
- `text`는 저신뢰 턴에 한해 이야기 어휘 근접 오인식 교정("방비" -> "방귀")이 끝난
  값이고 `rawText`가 벤더 원문이다. 발화 제출의 `sttRawText`에는 `rawText`를 되올린다
...
**4) 캐릭터 대사 재생**: `characterMessage.audioUrl`이 있으면 그대로 재생한다
(고정 첫/마지막 대사의 사전 렌더 음성, 2026-08-16부터). null이면
`POST /api/tts { text, characterName }` 호출 후 재생 - LLM이 만든 대사와 아이
이름이 치환된 문장은 항상 이 경로다. 재생이 끝나면 마이크 재활성화.
...
- 한때 계약만 있던 `POST /missions/{missionId}/result`는 **2026-08-15에 제거했다**
...
- 요소를 다 채워도 최소 턴(preferred_turns) 전이면 닫히지 않고 NORMAL로 이어진다.
  **원 자료의 요건이다** - 연동 기준 10.3과 11절이 "필수 요소 충족 및 최소 대화량
  충족"의 AND를 종료 조건으로 명시한다. 한때 충족조건 문서를 근거로 즉시 종료로
  바꿨다가(2026-08-15) 원 자료 요건 우선 원칙에 따라 복원했다(2026-08-16)
...
- **있는 이야기** (방귀 뀌는 며느리): post_activity_config에 카드와 핵심
  단어가 있다. 3.7절 흐름 그대로
...
콘텐츠에 config가 있으면 후속 활동이 돌고, 없으면 건너뛴다. 진행 가능한 방귀 뀌는
며느리는 갖고 있어 정상 흐름은 3.7절 그대로이고, 새 이야기는 config 없이 등록해도
갇히지 않는다.

## 5.4 STORY 전용 이야기 (대화 장면 없음)

장면이 전부 STORY인 이야기의 프론트 관점 흐름이다. 세션 시작 -> story-complete 반복 ->
마지막 응답에서 phase=POST_ACTIVITY, currentScene=null -> 후속 활동 -> 완료.
**발화, 미션, 대화 관련 코드가 전혀 타지 않는다.**

이 경우를 확인하려고 R__2에 "작은 씨앗"이라는 임시 이야기를 뒀었다. 대화 턴 파이프라인이
501이던 시절 대화를 거치지 않고 후속 활동까지 가 보는 유일한 길이었다. 파이프라인이 붙어
방귀 뀌는 며느리로 같은 흐름을 다 볼 수 있게 됐고, PUBLISHED라 실제 사용자 목록에도
노출되고 있어서 삭제했다(V19). 이 이야기의 존재에 기대는 코드나 테스트를 새로 만들지 않는다.
...
| 422 | STT_EMPTY_TEXT | 무음/인식 실패, 어휘 힌트 에코, 한글이 전혀 없는 결과(영어 환각) | 다시 말하기 안내 (sttRetryCount 증가) |
...
- 완료 기준: 방귀 뀌는 며느리의 STORY 장면 구간을 시작부터 첫 대화 장면 직전까지 통과

**2단계. 후속 활동** (3.7)
- [ ] start 카드 표시, order 제출과 재시도, keywords 표시
- [ ] retelling 제출과 완료 화면 (별가루, 해금 아이템)
- 완료 기준: 방귀 뀌는 며느리를 완주해 별가루가 지급된다
...
장면 영상(2026-08-16부터)도 같은 방식이다. 응답의 `videoUrl` 값 그대로 쓴다
(`/stories/banggui/scenes/01_intro.mp4` 등, 1280x720 H.264 무음). 방귀 뀌는 며느리
장면 1/2/3/4/5/6/8에 있고 7/9는 null(이미지 폴백)이다. 대화 장면 3과 5는 5초본
반복 시 이음매가 튀어 10초 루프본(`03_dialogue1_loop.mp4`, `05_dialogue2_loop.mp4`)을
쓴다.
...
| preferred_turns | 최소 대화량 게이트는 원 자료의 요건(연동 기준 10.3, 11절 - "필수 요소 충족 및 최소 대화량 충족"의 AND). 수치만 "운영 정책에 따라 관리"로 위임됨 | **전 장면 2로 확정** - 역할이 "한 발화에 요소를 다 채워도 최소 두 번은 주고받게 하는 하한"뿐이라 장면별로 다를 이유가 없다. 분량은 요소 4개와 max_turns가 결정하고, 이 값을 올리면 잘하는 아이에게 유도 불가능한 무목적 턴만 생긴다. 한때 충족조건 문서를 근거로 게이트를 제거했다가 원 자료 요건 우선으로 복원했다(2026-08-16) |
...
- 장면별 인정 기준(element_criteria)과 남은 걱정(remaining_worries)도 문서에 없어
  ...
  - 2026-08-16 걱정 문안 9건 개정(유도-판정 정합: 교차 배선·예/아니오형·정답 선점 제거,
    SeedWorryGuardTest로 금지선 고정). 여전히 콘텐츠팀 검수 전 제안값이다
