...
| 1 도입 | STORY | - | - | - | - |
| 2 전개1 | STORY | - | - | - | - |
| 3 대화1 | DIALOGUE | 방귀쟁이 며느리 | 2/4 | PERSPECTIVE, EMOTION, SOLUTION | 없음 |
| 4 전개2 | STORY | - | - | - | - |
| 5 대화2 | DIALOGUE | 시아버지 | 3/5 | PERSPECTIVE, EMPATHY, REASON, REQUEST | 없음 |
| 6 전개3 | STORY | - | - | - | - |
| 7 대화3 | DIALOGUE | 마을 이장 | 3/5 | SOLUTION, REASON, REQUEST, RESULT | 미션1 (문제 해결) |
| 8 전개4 | STORY | - | - | - | - |
...
body: {
  ...
  "sttConfidence": 0.87,            // 선택. 0~1. 현재 STT 응답에 신뢰도가 없으므로 생략한다
  "sttRetryCount": 1,               // 선택. 다시 말한 횟수 (422 재시도마다 +1)
...
| 검사 순서 | 조건 | 의미 | 프론트 동작 |
|---|---|---|---|
| 1 | `sceneTransition != null` | 장면 종료 | `characterMessage`(마지막 대사) 재생 -> 3.6절 |
...
- `childMessage.sttLowConfidence`: 현재 항상 false(기준값 미정). 값이 생기면
  "잘 못 알아들었을 수 있어요" 안내에 쓴다
...
**에러 처리**는 6장 표를 따른다. 특히 응답을 못 받았을 때(409 CONCURRENT_TURN,
타임아웃, 500)는 재전송 전에 반드시 상태를 다시 읽는다:
...
mission: {
  ...
  title, description,
  payload: {
    questions: [{key, label}] 또는 null,   // 문제 해결형: tool, safety, request, expectedResult 4개
...
1. `characterMessage`(고정 마지막 대사)를 재생한다. 평가가 아니라 다음 전개로 잇는
   이야기 연결 대사다
2. `closingReason`(GOAL_MET / MAX_TURNS)으로 연출을 다르게 할 수 있지만,
   **아이에게 성공/실패나 평가처럼 보여주지 않는다.** MAX_TURNS도 이야기는 자연스럽게
   이어진다
3. **결과 연출**: 대화3(이장)의 마지막 대사 후에는 며느리의 방귀로 배가 떨어지는
   결과 연출이 이어진다(콘텐츠 규칙). 연출 이미지는 `/stories/banggui/scenes/07_result.jpg`
   이고, 이 장면에서만 마지막 대사 재생 뒤 연출을 끼워 넣은 다음 전환한다
4. `next`로 분기:
...
[트랜잭션 2: 아이 턴 반영] - 커밋되면 턴은 되돌릴 수 없다
  ...
  캐릭터 LLM 호출 (트랜잭션 밖, 약 3초)
      고정 마무리 대사가 있는 CLOSING 턴은 호출 생략
...
2. 마무리 대사 결정: 장면에 고정 마지막 대사(character_closing)가 있으면 그대로
   (LLM 호출 없음, 감정 null), 없으면 LLM이 생성. 시드의 대화 장면 4개는 전부 고정
   대사가 있다
3. 마무리 대사를 **끝나는 장면 소속으로** 저장(이름 치환 포함)
...
- **있는 이야기** (방귀 뀌는 며느리, 작은 씨앗): post_activity_config에 카드와 핵심
  단어가 있다. 3.7절 흐름 그대로
- **없는 이야기** (config가 null): 마지막 장면이 끝나면 세션은 POST_ACTIVITY로
  전환되지만, start/order가 404("후속 활동 구성이 없는 이야기입니다")를 던지고
  retelling도 막혀 **세션이 POST_ACTIVITY에서 빠져나갈 수 없다.**
  후속 활동을 건너뛰고 완료시키는 분기는 현재 서버에 없다

**따라서 콘텐츠 등록 시 post_activity_config는 사실상 필수다.** 현재 시드 두 이야기는
모두 갖고 있어 정상 흐름에서는 문제가 없지만, 새 이야기를 등록할 때 반드시 확인해야
한다. (이 갇힘 상태의 처리 방침은 팀 확정 필요 항목이다)
...
| 대화3 결과 연출 | `/stories/banggui/scenes/07_result.jpg` | **경로 규약** - 3.6절 참고 |
...
# 10. 콘텐츠 문서와 시드의 차이 (팀 확정 필요)

최종 콘텐츠 문서([기획/이야기_방귀_뀌는_며느리.md](기획/이야기_방귀_뀌는_며느리.md))와
시드를 전수 대조한 결과다. **프론트 구현에는 영향이 없다** - 서버가 내려주는 값이
기준이고, 콘텐츠가 확정되면 시드만 바뀐다. 아래는 콘텐츠팀과 백엔드가 확정해야 할
목록이다.

## 10.1 동작이 달라지는 것 (우선 확정)

| 항목 | 문서 | 시드(현재 동작) |
|---|---|---|
| 대화1 필수 요소 | 3절: PERSPECTIVE, EMOTION, **REASON**, SOLUTION / 5절: **EXPRESSION** 포함 (문서 내부 모순) | PERSPECTIVE, EMOTION, SOLUTION 3개 (4번째 요소 제외) |
| 대화2 필수 요소 | 3절: PERSPECTIVE, **EMOTION**, REASON, **SOLUTION** / 5절: PERSPECTIVE, **EMPATHY**, REASON, **REQUEST** (내부 모순) | 5절 채택 (EMPATHY, REQUEST) |
| 미션1 확인 요소 2번 | 4절: "어디로 피해야 할지"(대피) / 5절: "왜 그 방법이 가능한지"(가능성) (내부 모순) | 5절 채택. 단 질문 key가 `safety`로 남아 label과 어긋남. 대피는 REQUEST 인정 기준에 흡수됨 |

필수 요소는 진행 판단(장면 종료 시점)에 직접 쓰이므로 확정이 서두르는 편이 좋다.
대화1을 3절(REASON)로 확정하면 시드에 REASON과 그 인정 기준, 남은 걱정을 추가해야 한다.

## 10.2 문구 차이 (기록, 확정 시 시드 반영)

- 대화3 첫 대사 어미: 문서 3절 "없었소" / 5절과 시드 "없었단다" (같은 장면 마지막
  대사는 양쪽 다 하오체 "고맙소"라 문체가 섞여 있다)
- scene_goal 4건: 시드가 요소 구성에 맞춰 재작성했다. 대화1의 "솔직하게 말할 용기를
  준다", 대화3의 "안전하게"가 goal에서 빠졌다(분석 LLM 입력이므로 판정에 영향 가능)
- 미션 노출 원칙 문구가 시드에서 일부 축약됐다(의미 변화 없음)
- 후속 활동 카드 5장과 핵심 단어 4개는 **문서에 값이 없어 시드에서 창작**했다.
  콘텐츠팀 검수 필요
- 장면별 인정 기준(element_criteria), 남은 걱정(remaining_worries), 페르소나,
  최소 턴(preferred_turns = max - 2 제안값)도 문서에 없어 시드에서 작성했다.
  전부 LLM 프롬프트에 들어가는 텍스트라 콘텐츠팀 검수를 권한다
