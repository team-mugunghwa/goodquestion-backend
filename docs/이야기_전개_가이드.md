...
**알려진 갇힘 (수정 예정)**: 판정 순서상 요소를 다 채운 턴은 항상 GOAL_MET이 되므로
(MAX_TURNS 판정에 도달하지 않는다), "요소 전부 충족 + 미션 미완료" 상태로 마지막
턴(턴 수 == max_turns)에 도달하면 보류가 걸려 장면이 닫히지 않은 채 턴 수만 최대치가
된다. 이후 발화는 전부 진입 검증의 409 MAX_TURNS_EXCEEDED에 걸려 **미션 수행 발화조차
보낼 수 없고 장면을 빠져나갈 방법이 없다**(stop만 가능). 서버 수정 대상으로 등록돼
있으며, 수정 전까지 프론트가 회피할 방법은 없다.
...
1. 목표 달성 여부를 최종 계산해 기록. 판정 순서상 요소를 다 채운 턴은 항상 GOAL_MET이
   되므로, **MAX_TURNS로 닫히는 턴은 언제나 부족 요소가 남아 있고 goalMet=false다**
...
| MAX_TURNS 종료 | 동일 | MAX_TURNS로 판정되면 미션 미완료여도 닫힘. 단 요소를 다 채운 채 최대 턴에 닿으면 갇힘 버그(4.6절) |
...
| | GOAL_MET | MAX_TURNS |
|---|---|---|
| 조건 | 필수 요소 전부 + 최소 턴 이상 (+ 미션 완료) | 턴 수가 최대치 도달 (이때 요소는 항상 미충족 - 다 채웠다면 GOAL_MET이 먼저 걸린다) |
...
| 409 | MAX_TURNS_EXCEEDED | 최대 턴을 소진했는데 장면이 닫히지 못한 상태의 발화 (부분 커밋 후 재전송, 미션 갇힘). **정상적으로 닫힌 장면에 재전송하면 이 에러가 아니라 다음 장면의 턴으로 조용히 접수될 수 있다** - 그래서 재전송 전 3.4절 판정 표가 필수다 | turn-state나 resume으로 현재 위치 확인 |
